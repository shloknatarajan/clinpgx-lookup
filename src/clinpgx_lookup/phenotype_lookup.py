...
def lookup_phenotype_ids(
    ...
    *,
    ...
    if index is None:
        index = _load_index()
    ids, matched_names, scores = lookup_ids(phenotype_term, index=index, threshold=threshold, top_k=top_k)
    return ids, matched_names, scores


def phenotype_lookup(phenotype_term: str, threshold: float, top_k: int) -> Tuple[List[str], List[str], List[float]]:
...
def main() -> None:
    ...
    try:
        ...
        threshold = 0.6
        top_k = 5
    ids, matched_names, scores = lookup_phenotype_ids(term, threshold=threshold, top_k=top_k)
