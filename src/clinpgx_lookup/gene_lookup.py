...
Goal: gene name or symbol -> PharmGKB accession id(s)

Sources: data/genes/genes.tsv
Name columns used:
- primary: Name, Symbol
- lists: Alternate Names, Alternate Symbols
"""
...
def lookup_gene_ids(
    ...
    *,
    ...
    if index is None:
        index = _load_index()
    ids, matched_names, scores = lookup_ids(gene_term, index=index, threshold=threshold, top_k=top_k)
    return ids, matched_names, scores


def gene_lookup(gene_term: str, threshold: float, top_k: int) -> Tuple[List[str], List[str], List[float]]:
    return lookup_gene_ids(gene_term, threshold=threshold, top_k=top_k)


def main() -> None:
    import sys
    if len(sys.argv) < 2:
        print("Usage: python -m clinpgx_lookup.gene_lookup <gene term> [threshold] [top_k]")
        raise SystemExit(1)
    ...
    try:
        ...
        threshold = 0.6
        top_k = 5
    ids, matched_names, scores = lookup_gene_ids(term, threshold=threshold, top_k=top_k)
